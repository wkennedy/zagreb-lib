...
fn main() {
    ...
    } else if graph.is_likely_traceable() {
    ...
    println!("Current Zagreb index efficiency: {:.2}%", 100.0 * (zagreb_index as f64) / upper_bound);

    // Recommend network improvements
    recommend_network_improvements(&graph, &validator_names);
}
...
fn create_simulated_solana_network() -> (Graph, HashMap<usize, String>) {
    ...
    for i in 5..12 {
        ...
        if next_validator != i {
            graph.add_edge(i, next_validator).expect("Failed to add edge");
        }

        let next_next_validator = (i + 2) % 7 + 5;
        if next_next_validator != i {
            graph.add_edge(i, next_next_validator).expect("Failed to add edge");
        }
    }
    ...
    let additional_edges = [
        (3, 15), (7, 18), (2, 14), (9, 16), (1, 19),
        (8, 13), (4, 11), (6, 17), (0, 12), (5, 10)
    ];
